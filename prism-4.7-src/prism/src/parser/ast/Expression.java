	...
	public static boolean containsMultiObjective(Expression expr)
	{
		try {
			ASTTraverse astt = new ASTTraverse()
			{
				...
				{
					if (e.getNameCode() == ExpressionFunc.MULTI || e.getNameCode() == ExpressionFunc.MLESSMULTI || e.getNameCode() == ExpressionFunc.UNICHAIN || e.getNameCode() == ExpressionFunc.DETMULTI)
						throw new PrismLangException("Found one", e);
				}
			};
			expr.accept(astt);
