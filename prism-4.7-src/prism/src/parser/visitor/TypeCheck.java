...
public class TypeCheck extends ASTTraverse
{
	...
	public void visitPost(ExpressionFunc e) throws PrismLangException
	{
		...
		switch (e.getNameCode()) {
			...
			case ExpressionFunc.UNICHAIN:
			case ExpressionFunc.DETMULTI:
			case ExpressionFunc.MLESSMULTI:
				...
				int index = 0;
				if(e.getNameCode() == ExpressionFunc.MLESSMULTI) {
					index ++;
				}
				for (; index < n; index++) {
					if (!(types[index] instanceof TypeBool || types[index] instanceof TypeDouble)) {
						throw new PrismLangException("Type error: non-Boolean/Double argument to  function \"" + e.getName()
								+ "\"", e.getOperand(index));
					}
					if (seenBoolean && types[index] instanceof TypeDouble) {
					...
					if (types[index] instanceof TypeBool) {
						seenBoolean = true;
					}
				}
				break;
