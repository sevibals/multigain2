...
public class MDPModelChecker extends ProbModelChecker {
	...
	@Override
	protected StateValues checkProbPathFormulaLTL(Model model, Expression expr, boolean qual, MinMax minMax, BitSet statesOfInterest)
	...
	protected StateValues checkExpressionMultiObjective(Model model,
		...
		//TODO RAMC : Important method
		boolean memoryless = false;
		boolean unichain = false;
		boolean deterministic = false;
		Double accFrequency = null;
		if (((ExpressionFunc) expr).getName().equals("multi"))
			memoryless = false;
		else if (((ExpressionFunc) expr).getName().equals("mlessmulti"))
			memoryless = true;
		else if (((ExpressionFunc) expr).getName().equals("unichain"))
			unichain = true;
		else if (((ExpressionFunc) expr).getName().equals("detmulti"))
			deterministic = true;
		...
		var steadyStateSpecification = new ArrayList<SteadyStateConstraint>();
		LtlSpecification ltlSpecification = null;

		final var numericalObjectives = new ArrayList<Pair<Integer, String>>();
		//extract data
		for (int i = 0; i < numObjectives; i++) {
			if (expr.getOperand(i) instanceof ExpressionReward) {
				ExpressionReward rewardExpression = (ExpressionReward) expr.getOperand(i);
				RelOp relOp = rewardExpression.getRelOp();

				//check that the parameters are of the form we can handle
				if (!(rewardExpression.getExpression() instanceof ExpressionTemporal))
					throw new PrismException("The reward subexpression must be a temporal expression.");
				ExpressionTemporal eT = (ExpressionTemporal) rewardExpression.getExpression();
				...
				Operator op;
				if (relOp == RelOp.MAX) {
					numericalObjectives.add(new Pair<>(i, rewardExpression.getRewardStructIndex().toString()));
					op = Operator.R_MAX;
				} else if (relOp == RelOp.GT) // currently do not support
					//relOps.add(1);
					...
					op = Operator.R_GE;
					if (deterministic)
						throw new PrismException("Only numerical reward structures may be defined when calculating a deterministic policy.");
				} else if (relOp == RelOp.MIN) {
					numericalObjectives.add(new Pair<>(i, rewardExpression.getRewardStructIndex().toString()));
					op = Operator.R_MIN;
				} else if (relOp == RelOp.LT) // currently do not support
					throw new PrismException("Multi-objective properties can not use strict inequalities on P/R operators");
				...
				// Store rewards bound
				Expression rb = rewardExpression.getReward();
				...
				Object rs = rewardExpression.getRewardStructIndex();
				...
				if(ltlSpecification != null) {
					throw new PrismException("Only one P operator is allowed!");
				}

				final var op = ((ExpressionProb) expr.getOperand(i)).getRelOp();
				if (op == RelOp.MAX || op == RelOp.MIN) {
					throw new PrismException("Not allowed");
				}

				final var ltlThreshold = ((ExpressionProb) expr.getOperand(i)).getBound().evaluateDouble();
				final var ltlExpr = ((ExpressionProb) expr.getOperand(i)).getExpression();
				final var ltlModelChecker = new LTLModelChecker(this);
				final var mdpProduct = ltlModelChecker.constructProductMDP(this,(MDP) model, ltlExpr, null, AcceptanceType.RABIN);
				ltlSpecification = new LtlSpecification(ltlThreshold, mdpProduct);
			} else if (expr.getOperand(i) instanceof ExpressionSS) {
				final ExpressionSS ssExpression = (ExpressionSS) expr.getOperand(i);
				final var bound = ssExpression.getBound().evaluateDouble();
				final var op = ssExpression.getRelOp();
				String expression = null;
				if (ssExpression.getExpression() instanceof ExpressionLabel) {
					expression = ((ExpressionLabel) ssExpression.getExpression()).getName();
				}
				steadyStateSpecification.add(new SteadyStateConstraint(expression, op, bound));
			} else if (expr.getOperand(i) instanceof ExpressionLiteral && memoryless) {
				final var boundExpr = (ExpressionLiteral) expr.getOperand(i);
				try {
					accFrequency = Double.parseDouble(boundExpr.getString());
				}
				catch (NumberFormatException e) {
					throw new PrismException("Error when reading memory bound");
				}
			} else {
				throw new PrismException("Only long-run rewards, steady states and LTL specifications are supported.");
			}
		}

		String method = this.settings.getString(PrismSettings.PRISM_MDP_MULTI_SOLN_METHOD);
		IMultiLongRun mlr = null;
		if (deterministic) {
			mlr = new MultiLongRunPure((MDP) model, rewards, steadyStateSpecification,  ltlSpecification, method);
		}
		else if (ltlSpecification == null) {
			mlr = new MultiLongRun((MDP) model, rewards, operators, bounds, steadyStateSpecification, ltlSpecification, accFrequency, method);
		} else {
			mlr = new MultiLongRun(ltlSpecification.getMdpProduct().getProductModel(), rewards, operators, bounds, steadyStateSpecification, ltlSpecification, accFrequency, method);
		}
		mlr.createMultiLongRunProgram();

		StateValues sv = null;
		if (numericalObjectives.size() > 0 && memoryless) {
		...
		else if (numericalObjectives.size() == 0 && memoryless) {
			if (unichain) {
				throw new PrismException("Unichain solution may only be found for strategies disregarding memory");
			}
			sv = ((MultiLongRun) mlr).solveMemoryless();
		} else if (numericalObjectives.size() < 2) {
			if (unichain) {
				sv = ((MultiLongRun) mlr).solveUnichain();
			} else {
				sv = mlr.solveDefault();
			}
		} else {//Pareto
			if (deterministic) {
				throw new PrismException("Deterministic solution may only be computed with a single or no numerical reward structure.");
			}
			if (unichain) {
				throw new PrismException("Unichain solution may only be found for non-numerical or single-numerical queries");
			}
			final var paretoChecker = new ParetoChecker(this, ((MultiLongRun) mlr)::solveMulti);
			final var tileList = paretoChecker.approximatePareto(numericalObjectives.size(), maxIters);
			String paretoFile = settings.getString(PrismSettings.PRISM_EXPORT_PARETO_FILENAME);
			if (paretoFile != null && !paretoFile.equals("")) {
				paretoChecker.exportPareto(tileList, expr.toString(), numericalObjectives.stream().map(Pair::getValue).collect(Collectors.toList()), paretoFile);
			}
			sv = new StateValues(TypeVoid.getInstance(), tileList, model);
        }
		if (genStrat) {
			Strategy strat = mlr.getStrategy();
			this.result.setStrategy(strat);
		}
		return sv;
	}
	 ...
	 */
	protected StateValues checkRewardCoSafeLTL(Model model, Rewards modelRewards, Expression expr, MinMax minMax, BitSet statesOfInterest)
			throws PrismException {
		LTLModelChecker mcLtl;
	...
	/**
	 * Compute next=state probabilities. i.e. compute the probability of being in a state in {@code target} in the next step.
	 *
	 * @param mdp    The MDP
	...
	/**
	 * Given a value vector x, compute the probability: v(s) = min/max sched [ Sum_s' P_sched(s,s')*x(s') ]  for s labeled with a, v(s) = 0
	 * for s not labeled with a.
	 * <p>
	...
	/**
	 * Compute until probabilities. i.e. compute the min/max probability of reaching a state in {@code target}, while remaining in those in
	 * {@code remain}.
	 *
	 * @param mdp    The MDP
	...
	/**
	 * Compute reachability/until probabilities. i.e. compute the min/max probability of reaching a state in {@code target}, while remaining
	 ...
	 * @param known  Optionally, a set of states for which the exact answer is known Note: if 'known' is specified (i.e. is non-null, 'init'
	 *               must also be given and is used for the exact values). Also, 'known' values cannot be passed for some solution methods,
	 *               e.g. policy iteration.
	 */
	public ModelCheckerResult computeReachProbs(MDP mdp, BitSet remain, BitSet target, boolean min, double init[], BitSet known)
			throws PrismException {
		ModelCheckerResult res = null;
		BitSet no, yes;
		int n, numYes, numNo;
		...
		if (doIntervalIteration) {
			if (!min && (genStrat || exportAdv)) {
				throw new PrismNotSupportedException(
						"Currently, explicit engine does not support adversary construction for interval iteration and Pmax");
			}
			if (mdpSolnMethod != MDPSolnMethod.VALUE_ITERATION && mdpSolnMethod != MDPSolnMethod.GAUSS_SEIDEL) {
				throw new PrismNotSupportedException(
						"Currently, explicit engine only supports interval iteration with value iteration or Gauss-Seidel for MDPs");
			}
			if (init != null)
	...
	protected ModelCheckerResult computeReachProbsNumeric(MDP mdp, MDPSolnMethod method, BitSet no, BitSet yes, boolean min, double init[],
		...
		if (res == null) { // not yet computed, use iterationMethod
			if (!doIntervalIteration) {
				...
				res = doIntervalIterationReachProbs(mdp, no, yes, min, init, known, iterationMethod, getDoTopologicalValueIteration(),
						strat);
			}
		}
	...
	/**
	 * Prob0 precomputation algorithm. i.e. determine the states of an MDP which, with min/max probability 0, reach a state in
	 * {@code target}, while remaining in those in {@code remain}. {@code min}=true gives Prob0E, {@code min}=false gives Prob0A.
	...
	/**
	 * Prob1 precomputation algorithm. i.e. determine the states of an MDP which, with min/max probability 1, reach a state in
	 * {@code target}, while remaining in those in {@code remain}. {@code min}=true gives Prob1A, {@code min}=false gives Prob1E.
	...
	/**
	 * Compute reachability probabilities using value iteration. Optionally, store optimal (memoryless) strategy info.
	 *
	 * @param mdp   The MDP
	 ...
	 * @param strat Storage for (memoryless) strategy choice indices (ignored if null) Note: if 'known' is specified (i.e. is non-null,
	 *              'init' must also be given and is used for the exact values.
	 */
	protected ModelCheckerResult computeReachProbsValIter(MDP mdp, BitSet no, BitSet yes, boolean min, double init[], BitSet known,
														  int strat[])
			throws PrismException {
	 ...
	 * @param strat           Storage for (memoryless) strategy choice indices (ignored if null) Note: if 'known' is specified (i.e. is
	 *                        non-null), 'init' must also be given and is used for the exact values.
	 */
	protected ModelCheckerResult doValueIterationReachProbs(MDP mdp, BitSet no, BitSet yes, boolean min, double init[], BitSet known,
															IterationMethod iterationMethod, boolean topological, int strat[])
			throws PrismException {
		BitSet unknown;
		int i, n;
		double initVal;
		...
		if (topological) {
			...
			return iterationMethod.doTopologicalValueIteration(this, description, sccs, iteration, singletonSCCSolver, timer,
					iterationsExport);
		} else {
			// run the actual value iteration
	...
	/**
	 * Compute reachability probabilities using interval iteration. Optionally, store optimal (memoryless) strategy info.
	 *
	 * @param mdp             The MDP
	 * @param no              Probability 0 states
	 * @param yes             Probability 1 states
	 ...
	 *                        non-null, 'init' must also be given and is used for the exact values.
	 */
	protected ModelCheckerResult doIntervalIterationReachProbs(MDP mdp, BitSet no, BitSet yes, boolean min, double init[], BitSet known,
															   IterationMethod iterationMethod, boolean topological, int strat[])
			throws PrismException {
		BitSet unknown;
		int i, n;
		...
		IterationMethod.IterationIntervalIter below =
				iterationMethod.forMvMultMinMaxInterval(mdp, min, strat, true, enforceMonotonicFromBelow, checkMonotonic);
		IterationMethod.IterationIntervalIter above =
				iterationMethod.forMvMultMinMaxInterval(mdp, min, strat, false, enforceMonotonicFromAbove, checkMonotonic);
		...
		if (topological) {
			...
			// run the actual value iteration
			return iterationMethod.doTopologicalIntervalIteration(this, description, sccs, below, above, singletonSCCSolver, timer,
					iterationsExport);
		} else {
			// run the actual value iteration
	 ...
	 *              'init' must also be given and is used for the exact values.
	 */
	protected ModelCheckerResult computeReachProbsGaussSeidel(MDP mdp, BitSet no, BitSet yes, boolean min, double init[], BitSet known,
															  int strat[])
			throws PrismException {
	...
	/**
	 * Compute reachability probabilities using policy iteration. Optionally, store optimal (memoryless) strategy info.
	 *
	 * @param mdp:  The MDP
	 * @param no:   Probability 0 states
	 * @param yes:  Probability 1 states
	...
	/**
	 * Construct strategy information for min/max reachability probabilities. (More precisely, list of indices of choices resulting in
	 * min/max.) (Note: indices are guaranteed to be sorted in ascending order.)
	 *
	 * @param mdp      The MDP
	...
	/**
	 * Compute bounded reachability probabilities. i.e. compute the min/max probability of reaching a state in {@code target} within k
	 * steps.
	 *
	 * @param mdp    The MDP
	 * @param target Target states
	...
	/**
	 * Compute bounded until probabilities. i.e. compute the min/max probability of reaching a state in {@code target}, within k steps, and
	 * while remaining in states in {@code remain}.
	 *
	 * @param mdp    The MDP
	 * @param remain Remain in these states (optional: null means "all")
	 * @param target Target states
	...
	/**
	 * Compute bounded reachability/until probabilities. i.e. compute the min/max probability of reaching a state in {@code target}, within
	 * k steps, and while remaining in states in {@code remain}.
	 *
	 * @param mdp     The MDP
	 * @param remain  Remain in these states (optional: null means "all")
	 * @param target  Target states
	 ...
	 */
	public ModelCheckerResult computeBoundedReachProbs(MDP mdp, BitSet remain, BitSet target, int k, boolean min, double init[],
													   double results[])
			throws PrismException {
		ModelCheckerResult res = null;
		BitSet unknown;
	...
	/**
	 * Compute expected cumulative (step-bounded) rewards. i.e. compute the min/max reward accumulated within {@code k} steps.
	 *
	 * @param mdp        The MDP
	 ...
	 */
	double computeReachRewardsMaxUpperBound(MDP mdp, MDPRewards mdpRewards, BitSet target, BitSet unknown, BitSet inf)
			throws PrismException {
		if (unknown.isEmpty()) {
	 ...
	 */
	double computeReachRewardsMinUpperBound(MDP mdp, MDPRewards mdpRewards, BitSet target, BitSet unknown, BitSet inf)
			throws PrismException {
		// inf and target states become trap states (with dropped choices)
		BitSet trapStates = (BitSet) target.clone();
		trapStates.or(inf);
	...
	/**
	 * Return true if the MDP is contracting for all states in the 'unknown' set, i.e., if Pmin=1( unknown U target) holds.
	 */
	...
	/**
	 * Compute upper bound for maximum expected reward (variant 1, coarse), i.e., does not compute separate q_t / p_t per SCC. Uses Rs = S,
	 * i.e., does not take reachability into account.
	 *
	 * @param mdp        the model
	 * @param mdpRewards the rewards
	 * @param target     the target states
	 ...
	 * @return upper bound on Rmax=?[ F target ] for all states
	 */
	double computeReachRewardsMaxUpperBoundVariant1Coarse(MDP mdp, MDPRewards mdpRewards, BitSet target, BitSet unknown, BitSet inf)
			throws PrismException {
	...
	/**
	 * Compute upper bound for maximum expected reward (variant 1, fine). i.e., does compute separate q_t / p_t per SCC. Uses Rs = S, i.e.,
	 ...
	 * @return upper bound on Rmax=?[ F target ] for all states
	 */
	double computeReachRewardsMaxUpperBoundVariant1Fine(MDP mdp, MDPRewards mdpRewards, BitSet target, BitSet unknown, BitSet inf)
			throws PrismException {
		double[] boundsOnExpectedVisits = new double[mdp.getNumStates()];
	...
	/**
	 * Compute upper bound for maximum expected reward (variant 2). Uses Rs = S, i.e., does not take reachability into account.
	 *
	 ...
	 */
	double computeReachRewardsMaxUpperBoundVariant2(MDP mdp, MDPRewards mdpRewards, BitSet target, BitSet unknown, BitSet inf)
			throws PrismException {
	...
	/**
	 * Compute expected instantaneous reward, i.e. compute the min/max expected reward of the states after {@code k} steps.
	 *
	 * @param mdp        The MDP
	 * @param mdpRewards The rewards
	...
	public ModelCheckerResult computeTotalRewardsMax(MDP mdp, MDPRewards mdpRewards, boolean noPositiveECs) throws PrismException {
		...
		// Switch to a supported method, if necessary
		if (!(mdpSolnMethod == MDPSolnMethod.VALUE_ITERATION
				|| mdpSolnMethod == MDPSolnMethod.GAUSS_SEIDEL
		...
		if (noPositiveECs) {
			...
			mainLog.println("Precomputation took " + timerPre / 1000.0 + " seconds, " + inf.cardinality() + " infinite states, " + (n
					- inf.cardinality()) + " states remaining.");
		}
	...
	/**
	 * Compute expected reachability rewards. i.e. compute the min/max reward accumulated to reach a state in {@code target}.
	 *
	 * @param mdp        The MDP
	 * @param mdpRewards The rewards
	 * @param target     Target states
	 * @param min        Min or max rewards (true=min, false=max)
	 * @param init       Optionally, an initial solution vector (may be overwritten)
	 * @param known      Optionally, a set of states for which the exact answer is known Note: if 'known' is specified (i.e. is non-null,
	 *                   'init' must also be given and is used for the exact values). Also, 'known' values cannot be passed for some
	 *                   solution methods, e.g. policy iteration.
	 */
		...
		if (numTarget + numInf < n) {
			...
			if (min & doZeroMECCheckForMin) {
				...
				if (quotient == null) {
					...
					if (strat != null) {
						throw new PrismException(
								"Constructing a strategy for Rmin in the presence of zero-reward ECs is currently not supported");
					}
				}
			}
			...
			if (quotient != null) {
				...
				res = computeReachRewardsNumeric(quotient.getModel(), quotient.getRewards(), mdpSolnMethod, target, newInfStates, min, init,
						known, strat);
	...
	protected ModelCheckerResult computeReachRewardsNumeric(MDP mdp, MDPRewards mdpRewards, MDPSolnMethod method, BitSet target, BitSet inf,
		...
		if (res == null) { // not yet computed, use iterationMethod
			if (!doIntervalIteration) {
				res = doValueIterationReachRewards(mdp, mdpRewards, iterationMethod, target, inf, min, init, known,
						getDoTopologicalValueIteration(), strat);
			} else {
				res = doIntervalIterationReachRewards(mdp, mdpRewards, iterationMethod, target, inf, min, init, known,
						getDoTopologicalValueIteration(), strat);
			}
		}
	...
	/**
	 * Compute expected reachability rewards using value iteration. Optionally, store optimal (memoryless) strategy info.
	 *
	 * @param mdp        The MDP
	 * @param mdpRewards The rewards
	 * @param target     Target states
	 ...
	 *                   'init' must also be given and is used for the exact values.
	 */
	protected ModelCheckerResult computeReachRewardsValIter(MDP mdp, MDPRewards mdpRewards, BitSet target, BitSet inf, boolean min,
	 ...
	 *                    non-null, 'init' must also be given and is used for the exact values.
	 */
	protected ModelCheckerResult doValueIterationReachRewards(MDP mdp, MDPRewards mdpRewards, IterationMethod iterationMethod,
															  BitSet target, BitSet inf, boolean min, double init[], BitSet known,
															  boolean topological, int strat[])
			throws PrismException {
		BitSet unknown;
		int i, n;
		long timer;
		...
		timer = System.currentTimeMillis();
		String description =
				(min ? "min" : "max") + (topological ? ", topological" : "") + ", with " + iterationMethod.getDescriptionShort();
		mainLog.println("Starting value iteration (" + description + ")...");
		...
		if (topological) {
			...
			// run the actual value iteration
			return iterationMethod.doTopologicalValueIteration(this, description, sccs, forMvMultRewMinMax, singletonSCCSolver, timer,
					iterationsExport);
		} else {
			// run the actual value iteration
	...
	/**
	 * Compute expected reachability rewards using Gauss-Seidel (including Jacobi-style updates). Optionally, store optimal (memoryless)
	 * strategy info.
	 *
	 * @param mdp        The MDP
	 * @param mdpRewards The rewards
	 ...
	 *                   'init' must also be given and is used for the exact values.
	 */
	protected ModelCheckerResult computeReachRewardsGaussSeidel(MDP mdp, MDPRewards mdpRewards, BitSet target, BitSet inf, boolean min,
																double init[],
																BitSet known, int strat[]) throws PrismException {
		IterationMethodGS iterationMethod = new IterationMethodGS(termCrit == TermCrit.ABSOLUTE, termCritParam, false);
		return doValueIterationReachRewards(mdp, mdpRewards, iterationMethod, target, inf, min, init, known, false, strat);
	...
	/**
	 * Compute expected reachability rewards using interval iteration Optionally, store optimal (memoryless) strategy info.
	 *
	 * @param mdp         The MDP
	 * @param mdpRewards  The rewards
	 * @param target      Target states
	 ...
	 *                    non-null, 'init' must also be given and is used for the exact values.
	 */
	protected ModelCheckerResult doIntervalIterationReachRewards(MDP mdp, MDPRewards mdpRewards, IterationMethod iterationMethod,
																 BitSet target, BitSet inf, boolean min, double init[], BitSet known,
																 boolean topological, int strat[])
			throws PrismException {
		BitSet unknown;
		...
		IterationMethod.IterationIntervalIter below =
				iterationMethod.forMvMultRewMinMaxInterval(mdp, mdpRewards, min, strat, true, enforceMonotonicFromBelow, checkMonotonic);
		IterationMethod.IterationIntervalIter above =
				iterationMethod.forMvMultRewMinMaxInterval(mdp, mdpRewards, min, strat, false, enforceMonotonicFromAbove, checkMonotonic);
		below.init(initBelow);
		above.init(initAbove);
		...
		if (topological) {
			...
			// run the actual value iteration
			rv = iterationMethod.doTopologicalIntervalIteration(this, description, sccs, below, above, singletonSCCSolver, timer,
					iterationsExport);
		} else {
			// run the actual value iteration
	 ...
	 * for policy iteration, and as storage for the resulting optimal strategy (if needed). Passing in an initial strategy is required when
	 * some states have infinite reward, to avoid the possibility of policy iteration getting stuck on an infinite-value strategy.
	 *
	 * @param mdp        The MDP
	 * @param mdpRewards The rewards
	 * @param target     Target states
	 ...
	 * @param strat      Storage for (memoryless) strategy choice indices (ignored if null)
	 */
	protected ModelCheckerResult computeReachRewardsPolIter(MDP mdp, MDPRewards mdpRewards, BitSet target, BitSet inf, boolean min,
															int strat[])
			throws PrismException {
		ModelCheckerResult res;
		int i, n, iters, totalIters;
	...
	/**
	 * Construct strategy information for min/max expected reachability. (More precisely, list of indices of choices resulting in min/max.)
	...
	/**
	 * Restrict a (memoryless) strategy for an MDP, stored as an integer array of choice indices, to the states of the MDP that are
	 * reachable under that strategy.
	 *
	 * @param mdp   The MDP
	...
	/**
	 * Compute the end component quotient (for use with PMax), each maximal end component is collapsed to a single state, likewise the yes
	 * and no regions, respectively.
	 */
