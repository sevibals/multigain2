...
public class NondetModelChecker extends NonProbModelChecker
{
	...
	protected StateValues checkExpressionMultiObjective(List<Expression> exprs, JDDNode statesOfInterest) throws PrismException
	{
		...
		for (int i = 0; i < numObjectives; i++) {
			extractInfoFromMultiObjectiveOperand((ExpressionQuant) exprs.get(i), opsAndBounds, transRewardsList, targetName, pathFormulas, i);
		}
	...
	//From Multigain
	protected void extractInfoFromMultiObjectiveOperand(ExpressionQuant expressionQuant, OpsAndBoundsList opsAndBounds, List<JDDNode> rewardsIndex, List<String> targetName,
														List<Expression> targetExprs, int origPosition) throws PrismException
	{
		RelOp relOp;
		int stepBound = 0;
		ExpressionProb exprProb = null;
		...
		OpRelOpBound opInfo = expressionQuant.getRelopBoundInfo(constantValues);
		// Only do P or R operators
		if (!(expressionQuant instanceof ExpressionProb || expressionQuant instanceof ExpressionReward))
		...
		if (expressionQuant instanceof ExpressionProb) {
			exprProb = (ExpressionProb) expressionQuant;
			exprReward = null;
		} else {
			exprReward = (ExpressionReward) expressionQuant;
			exprProb = null;
			Object rs = exprReward.getRewardStructIndex();
		...
		if (pb != null) {
			...
			opsAndBounds.add(opInfo, op, -1.0, stepBound, origPosition);
		}
